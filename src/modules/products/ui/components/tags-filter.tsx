                ...
                data?.pages.map((page) =>
                    page.docs.map((tag) => (
                        <div key={tag.id} className="flex items-center justify-between cursor-pointer" onClick={() => onClick(tag.name)}>
                            ...
                            <Checkbox className="cursor-pointer" checked={value?.includes(tag.name)} onCheckedChange={() => onClick(tag.name)}/>
                        </div>
                    )))
            )}
            {hasNextPage && (
                <button disabled={isFetchingNextPage} onClick={() => fetchNextPage()} className="underline cursor-pointer font-medium justify-start text-start disabled:opacity-50">
                    Load more...
                </button>
            )}
        </div>
